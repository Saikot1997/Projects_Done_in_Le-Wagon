...
def preprocess_features(X: pd.DataFrame):
    # Define the necessary encoders and preprocessors
    ...
    final_preprocessor = make_column_transformer(
        (passenger_pipe, ["passenger_count"]),
        (time_pipe, ["pickup_datetime"]),
        (distance_pipe, ["pickup_latitude", "pickup_longitude", "dropoff_latitude", "dropoff_longitude"]),
        (geohash_pipe, ["pickup_latitude", "pickup_longitude", "dropoff_latitude", "dropoff_longitude"])
    )
